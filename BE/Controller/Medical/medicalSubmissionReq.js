...
const updateMedicationSubmissionReqByNurse = async (req, res, next) => {
  ...
  const { status } = req.body; // Trạng thái mới: ACCEPTED hoặc DECLINED
  const pool = await sqlServerPool;
  ...
  try {
    ...
    if (result.rowsAffected[0] > 0) {
      ...
      const parentResult = await pool.request().input("id_req", sql.Int, ReqId)
        .query(`
      ...
      const parentEmailResult = await pool
        .request()
        .input("id_req", sql.Int, ReqId).query(`
          SELECT u.email
...
Rất tiếc, yêu cầu gửi thuốc của quý phụ huynh đã bị từ chối bởi y tá phụ trách.
