...
// Thunk cập nhật trạng thái sự cố y tế
export const updateMedicalIncidentStatus = createAsyncThunk(
  "medicalIncidents/updateStatus",
  async ({ event_id, status }, { rejectWithValue }) => {
    try {
      const response = await api.patch(`/nurse/incidents/student/${event_id}/update`, { status });
      return response.data;
    } catch (error) {
      return rejectWithValue(error.response?.data?.message || "Cập nhật trạng thái sự cố y tế thất bại.");
    }
  }
);
...
const medicalIncidentsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateMedicalIncidentStatus.pending, (state) => {
        state.loading = true;
        state.error = null;
        state.success = false;
      })
      .addCase(updateMedicalIncidentStatus.fulfilled, (state, action) => {
        state.loading = false;
        state.success = true;
        // Cập nhật record trong danh sách nếu có event_id
        const updatedId = action.meta.arg.event_id;
        const updatedStatus = action.meta.arg.status;
        if (updatedId && state.records && Array.isArray(state.records)) {
          state.records = state.records.map((item) =>
            item.event_id === updatedId ? { ...item, status: updatedStatus } : item
          );
        }
      })
      .addCase(updateMedicalIncidentStatus.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload;
        state.success = false;
      });
