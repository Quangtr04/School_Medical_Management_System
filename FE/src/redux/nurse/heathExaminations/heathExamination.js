...
export const fetchAllHealthExaminations = createAsyncThunk(
  ...
  async (_, { rejectWithValue }) => {
    ...
    try {
      // Đảm bảo endpoint này khớp với backend của bạn
      const response = await api.get("/nurse/checkups"); // Bỏ /api/ nếu config-axios đã có base URL
...
export const fetchHealthExaminationById = createAsyncThunk(
  ...
  async (id, { rejectWithValue }) => {
    try {
      const response = await api.get(`/nurse/checkups-approved/students/${id}`); // Bỏ /api/ nếu config-axios đã có base URL
...
export const updateStundentRecord = createAsyncThunk(
  "healthExaminations/updateStudentRecord",
  async ({ id, values }, { rejectWithValue }) => {
    console.log("Values nhận vào từ Form", values);
    try {
      const response = await api.patch(`/nurse/checkups/students/${id}/update`, values);
...
const healthExaminationsSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      //updateStudentRecord
      .addCase(updateStundentRecord.pending, (state, action) => {
        state.loading = true;
        state.error = null;
        state.success = false;
      })

      .addCase(updateStundentRecord.fulfilled, (state, action) => {
        state.loading = false;
        state.error = null;
        state.success = true;
      })

      .addCase(updateStundentRecord.rejected, (state, action) => {
        state.loading = false;
        state.error = action.payload;
      });
