...
export default function NurseManagementPage() {
  ...
  const handleAddNurse = () => {
    ...
    form.setFieldsValue({
      ...
      dayofbirth: null,
      password: "",
    }); // Set default status for new nurse
  ...
  const handleFormSubmit = async (values) => {
    ...
    try {
      const payload = {
        ...values,
        is_active: values.status === "Hoạt động",
      ...
      if (editingNurse) {
        dispatch(
          ...
          .then(() => {
            ...
            dispatch(fetchUsers({ endpointPath: "/admin/nurses" })); // Refresh bảng
            form.resetFields(); // Reset form
          })
          .catch((error) => {
        ...
        const resultAction = await dispatch(
          ...
          .then(() => {
            ...
            setIsModalVisible(false); // Đóng modal
            form.resetFields(); // Reset form
            dispatch(fetchUsers({ endpointPath: "/admin/nurses" })); // Cập nhật bảng
          })
          .catch((error) => {
