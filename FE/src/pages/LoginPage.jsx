  ...
  const onFinish = async (values) => {
    ...
    try {
      ...
      if (response.data && response.data.token && response.data.user) {
        ...
        const { role_id } = user; // <-- Lấy roleId từ đối tượng user trong response
        console.log(role_id);

        console.log(user);

        // 1. Lưu token và thông tin người dùng vào localStorage/Context
        localStorage.setItem("authToken", token);
        // Bạn cũng có thể lưu toàn bộ đối tượng user hoặc roleId vào localStorage nếu cần
        localStorage.setItem("currentUser", JSON.stringify(user));

        toast.success(message || "Đăng nhập thành công!");
        // 2. Điều hướng dựa trên roleId
        switch (role_id) {
          case 1: // Admin
          ...
          case 4: // Parent
            ...
            break;
          default:
            // Trường hợp roleId không xác định hoặc không khớp
