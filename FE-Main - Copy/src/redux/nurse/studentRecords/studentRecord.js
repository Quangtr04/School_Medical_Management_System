...
const studentHealthSlice = createSlice({
  ...
  extraReducers: (builder) => {
    builder
      ...
      .addCase(updateStudentHealthRecord.fulfilled, (state, action) => {
        state.loading = false;
        state.success = true;
        // Cập nhật bản ghi trong danh sách nếu có
        const updatedRecord = action.payload;
        const index = state.healthRecords.findIndex(
          (record) => record.id === updatedRecord.id // Giả sử ID là duy nhất
        );
        if (index !== -1) {
          state.healthRecords[index] = updatedRecord;
        }
        // Cập nhật currentHealthRecord nếu nó là bản ghi đang được xem
        if (state.currentHealthRecord && state.currentHealthRecord.id === updatedRecord.id) {
          state.currentHealthRecord = updatedRecord;
        }
      })
